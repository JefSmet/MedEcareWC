...
type
  ...
  TShiftTypeRate = record
    ...
    class function FromJSON(const AJson: string; ADateTimeIsUTC: Boolean): TShiftTypeRate; overload; static;
    class function FromJSON(const AJsonObj: TJSObject; ADateTimeIsUTC: Boolean): TShiftTypeRate; overload; static;
  end;

implementation

class function TShiftTypeRate.FromJSON(const AJson: string; ADateTimeIsUTC: Boolean): TShiftTypeRate;
begin
  Result := FromJSON(TJSJSON.parseObject(AJson), ADateTimeIsUTC);
end;

class function TShiftTypeRate.FromJSON(const AJsonObj: TJSObject; ADateTimeIsUTC: Boolean): TShiftTypeRate;
begin
  ...
  begin
    Result.ValidFrom := TWebRESTClient.IsoToDateTime(JS.toString(AJsonObj['validFrom']), ADateTimeIsUTC);
  end;
  if AJsonObj.hasOwnProperty('validUntil') then
  begin
    Result.ValidUntil := TWebRESTClient.IsoToDateTime(JS.toString(AJsonObj['validUntil']), ADateTimeIsUTC);
  end;
