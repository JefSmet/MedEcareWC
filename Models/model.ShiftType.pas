...
type
  TShiftType = record
    ...
    CreatedAt       : TDateTime;
    UpdatedAt       : TDateTime;
    class function FromJSON(const AJson: string): TShiftType; static;
  end;

implementation

class function TShiftType.FromJSON(const AJson: string): TShiftType;
var
  o: TJSObject;
begin
  ...
  if o.hasOwnProperty('activeFrom')      then Result.ActiveFrom      := TWebRESTClient.IsoToDateTime(string(o['activeFrom']));
  if o.hasOwnProperty('activeUntil')     then Result.ActiveUntil     := TWebRESTClient.IsoToDateTime(string(o['activeUntil']));
